...
cargo test               # run 17 insta snapshot tests
...
│   ├── build.rs            # `copperhead build` — transpile + cargo build
...
│   ├── annotations.rs      # Parse type annotations: own[T], borrow[T], mut[T], Optional, Result, etc.
│   └── pydantic.rs         # Recognize BaseModel subclasses, Field() constraints, @field_validator
├── ast/
│   ├── mod.rs              # Copperhead AST nodes: Module, Item, Statement, Expr, StructDef, FieldDef
│   └── types.rs            # CopperheadType enum + Ownership enum + TypeAnnotation
├── checker/                # Type + ownership checker (stub)
├── hir/                    # High-level IR (stub)
├── codegen/
│   ├── mod.rs              # Main codegen: structs, constructors, validators, methods
│   ├── expr.rs             # Expression codegen: builtins, string methods, f-strings, try_()
...
tests/
├── codegen_tests.rs        # 17 snapshot tests (insta)
...
├── customer_api/           # Source .cu.py files for the CRUD API
└── customer_api_generated/ # Hand-written reference Rust output (Actix-web + SQLite)
...
All 17 tests in `tests/codegen_tests.rs` use `insta::assert_snapshot!` with file-based snapshots.
To update snapshots: `cargo insta review`

Test coverage: hello_world, function_with_types, ownership_borrow, ownership_mut, pydantic_basic_struct, pydantic_with_constraints, pydantic_optional_with_default, pydantic_validator, fstring, for_range, while_loop, result_type, try_operator, list_operations, string_methods, if_else, async_function.
